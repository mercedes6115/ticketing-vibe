...
> 동시 접속이 몰리는 티켓팅 상황을 가정하고, 대기열 진입부터 좌석 선점, 비동기 예매 처리, 실시간 상태 반영까지 구현한 프로젝트입니다.

| 항목 | 내용 |
|------|------|
| 핵심 문제 | 동시성 제어, 상태 정합성, 비동기 예매 확정, 운영 중 병목 관찰 |
| 핵심 기술 | Spring Boot, Redis, Redisson, Kafka, React, WebSocket, SSE |
| 검증 방식 | k6 부하 테스트, Prometheus 메트릭, Grafana 대시보드 |
| 문서 포인트 | 설계 이유, 트레이드오프, 성능 실험 결과, AI 활용 범위 |

## 문서 안내

- [프로젝트 배경](#왜-이-프로젝트를-만들었는가)
- [문제 정의](#문제-정의)
- [핵심 설계](#핵심-설계)
- [기술 스택](#기술-스택)
- [성능 실험 요약](#성능-실험-요약)
...
- 이 프로젝트의 중심은 "예매 기능 구현"이 아니라 "실시간 경합 상태를 어떻게 안전하게 제어할 것인가"입니다.
- 대기열, 좌석 선점, 예매 확정, 모니터링을 서로 다른 책임으로 나눠 설계했습니다.
- 구현 후에는 부하 테스트와 병목 분석으로 결과를 수치화했고, 효과 없는 최적화는 롤백했습니다.
...
| 문서 | 설명 |
|------|------|
| [시스템 아키텍처](docs/architecture.md) | 전체 구성, Redis 키 구조, Kafka 토픽, 포트 정리 |
| [서비스 플로우](docs/service-flow.md) | 대기열, 좌석 선점, 예매, 인증 흐름의 내부 처리 순서 |
| [시퀀스 다이어그램](docs/sequence-diagram.md) | 사용자 기준 E2E 흐름을 단계별로 시각화한 문서 |
| [클래스 다이어그램](docs/class-diagram.md) | Entity, Service, Repository 관계 요약 |
| [API 문서](docs/api.md) | 주요 REST API 요청/응답 스펙 |
| [성능 테스트 보고서](docs/performance-report.md) | 실험 목적, 시나리오, 측정 기준, 결과 해석 |
| [설계 트레이드오프](docs/tradeoffs.md) | 대안 비교와 선택 이유, 감수한 비용 |
| [AI 활용 및 검증 기록](docs/ai-usage.md) | AI 활용 범위와 직접 검증한 판단 정리 |
| [ERD](ERD.md) | 데이터 모델 요약 |
