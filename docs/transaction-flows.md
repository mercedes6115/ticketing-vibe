...
분산락(Redisson) + DB 비관적 락(Pessimistic Lock)을 조합해 이중 선점을 방지한다.
...
Redis 홀드 키 삭제를 [`afterCommit() - afterCommit()이란?`](https://www.notion.so/afterCommit-36c05755fb87807d8ed2d7b70cf9a545?source=copy_link)으로 지연한다.
...
Consumer가 DB 쓰기를 담당한다. [`Idempotency - Idempotency는 뭘까?`](https://www.notion.so/Idempotency-36c05755fb87800eb69ac0b436ad2948?source=copy_link)를 위해 `bookingNo` 중복 체크로 at-least-once 재처리에 안전하게 만든다.
...
Kafka 발행을 [`afterCommit() - afterCommit()이란?`](https://www.notion.so/afterCommit-36c05755fb87807d8ed2d7b70cf9a545?source=copy_link)으로 지연해 롤백 시 CANCELLED 이벤트 오발행을 막는다.
...
리프레시 토큰 저장을 [`afterCommit() - afterCommit()이란?`](https://www.notion.so/afterCommit-36c05755fb87807d8ed2d7b70cf9a545?source=copy_link)으로 지연해 DB 롤백 시 7일 토큰 잔류를 방지한다.
...
사용자가 선점 후 5분 내 예매하지 않으면 [Redis TTL / Keyspace Notification - 처음 들어보는 Redis TTL / Keyspace Notification?](https://www.notion.so/Redis-TTL-Keyspace-Notification-36c05755fb87800cb4cfef7e3ba08be3?source=copy_link) 만료 이벤트로 좌석을 자동 해제한다.
