...
이 문서는 실시간 티켓팅 시스템의 부하 테스트 결과를 기록하기 위한 문서다.

목적은 단순히 "테스트를 돌렸다"를 남기는 것이 아니라, 어떤 병목을 검증하려고 했는지, 어떤 기준으로 성공과 실패를 판단하는지, 실제 결과가 그 기준을 만족하는지를 수치로 설명하는 것이다.
...
- ??? / ????????(66,704?) ???? ???? ?? ??? ?? k6 ????? ????.
- ??? API? ??? ?? ?? ??? ??, ?? ???? ? ??? `accessToken`? ?? ???? k6 `setup()`? ??? ?? ??? ????? ????.
- ?? `users=200000` ?? ???? ?????, ???? ?? ?? ?? **k6 ??? ??**?? ?? ????.

### ?? ??

- ?? ??? ?????? ?? ??? ?? ?????? **?? ?? ??? ? ?? ??? ??**? ?? ??? ??? ????.
- ?? ??? `full_flow_test_data.json` ??? ? ?? `open() + JSON.parse()`? ????, ??? ??/?? ???? ??? ???? ??? ? ???? ????.
- ??? ??? ?? ??? `setup()` ??? ??? ?? ??? ????, **??? ??? ???? ? ?????? ?? ???? ??**? ??? ?? ??.
- ??? `20?`, `50?` ?? ????? ?? k6 ????? ?? ????, ???? ?? ?? ??? ??? ??? ???? ??.
