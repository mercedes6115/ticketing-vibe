...
flowchart TD
    ...
    B -->|없음| C["400 토큰 없음 / 대기열 필요"]
    B -->|있음| D["Redisson RLock 획득 시도<br/>seat:lock:&lt;seatId&gt;<br/>대기 3초 / 임대 10초"]
    D -->|타임아웃| E["409 다른 사용자 선점 중"]
    D -->|획득 성공| F["DB findByIdWithEvent<br/>seat + event 조회"]
    F --> G{"seat.status"}
    ...
    J --> K["RLock 해제"]
    K --> L["트랜잭션 커밋"]
    L --> M["afterCommit callback<br/>WebSocket 브로드캐스트<br/>topic/events/&lt;eventId&gt;/seats"]
...
flowchart TD
    ...
    subgraph ASYNC["비동기 처리 (BookingRequestConsumer)"]
        ...
        K -->|있음| L["idempotent skip<br/>이미 처리된 요청"]
...
- `DefaultErrorHandler`: 시스템 예외는 1초 간격 3회 재시도 후 `.DLQ` 격리
...
flowchart TD
    ...
    subgraph REISSUE["토큰 재발급"]
        I["POST /api/auth/reissue<br/>refreshToken"] --> J{"Redis refresh:&lt;userId&gt; 일치?"}
        J -->|불일치| K["401 유효하지 않은 토큰"]
        J -->|일치| L["기존 토큰 삭제"]
        L --> M["새 accessToken + refreshToken 발급"]
        M --> N["Redis 갱신<br/>refresh:&lt;userId&gt; = 새 refreshToken"]
    end
```
